...
public class MultiSelectionSpinner extends AppCompatSpinner implements
    ...
    public boolean performClick() {
        ...
        for (int i = 0; i < items.size(); i++) {
            itemNames[i] = items.get(i).getTema();
        }
    ...
    public void setItems(ArrayList<Study> items) {
        this.items = items;
    ...
    public void setSelection(ArrayList<Study> selection) {
        ...
        for (Study sel : selection) {
            for (int j = 0; j < items.size(); ++j) {
                if (items.get(j).getTema().equals(sel.getTema())) {
    ...
    public ArrayList<Study> getSelectedItems() {
        ArrayList<Study> selectedItems = new ArrayList<>();
